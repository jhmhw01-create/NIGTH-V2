...
React 전환 2단계: HOME·DISCOGRAPHY·CONTENTS·NOTICE와 해당 페이지의 메뉴·푸터는 React로 동작합니다. 콘텐츠 분류 및 공지의 분류·연도 필터는 React 상태로 처리합니다. 빌드 시 본문을 미리 렌더링하므로 기존 주소와 상세페이지 연결을 유지합니다. 다른 48개 페이지는 기존 HTML 생성 방식을 유지합니다. 전체 React SPA 전환은 아직 아닙니다.
