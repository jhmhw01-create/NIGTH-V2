...
export function StageLightbox({variant,photo,index,total,label,onClose,onMove}) {
  ...
  useEffect(()=>{
    ...
    const lock=native?'sg-lock':'fm-lightbox-open';
  ...
  return <div ref={viewer} id="fmLightbox" className={'fm-lightbox'+(open?' is-open':'')} role="dialog" aria-modal="true" aria-label={label} aria-hidden={!open} onClick={backdrop}>
    <button className="fm-lightbox-close" type="button" aria-label="닫기" onClick={onClose}>×</button><button className="fm-lightbox-nav fm-lightbox-prev" type="button" aria-label="이전 이미지" onClick={()=>onMove(-1)}>‹</button><div className="fm-lightbox-stage"><img key={photo?.full??'empty'} className="fm-lightbox-image" src={photo?.full} alt={photo?.alt??''} onError={imageError}/><div className="fm-lightbox-meta"><span className="fm-lightbox-title">{photo?.title??''}</span><span className="fm-lightbox-index">{open?`${index+1} / ${total}`:''}</span></div></div><button className="fm-lightbox-nav fm-lightbox-next" type="button" aria-label="다음 이미지" onClick={()=>onMove(1)}>›</button>
  </div>;
}
