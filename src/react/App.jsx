...
export function App({route='index.html'}) {return <><Header activeNav={albumRoutes.includes(route)?'DISCOGRAPHY':stageRoutes.includes(route)||storyRoutes.includes(route)?'CONTENTS':fanclubDetailRoutes.includes(route)?'FANCLUB':route==='archive.html'?'ARCHIVE':route.startsWith('member-')?'MEMBERS':active[route]} />{route==='index.html'?<HomePage />:route==='archive.html'?<SearchArchive />:<ArchivePage route={route} />}<Footer /></>;}
