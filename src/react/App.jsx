...
function SiteApp({route='index.html'}) {return <><Header activeNav={route==='store.html'?'FANCLUB':route==='highlight-medley.html'?'LISTEN':route==='press.html'?'ARCHIVE':route==='five-voices.html'?'CONTENTS':albumRoutes.includes(route)?'DISCOGRAPHY':stageRoutes.includes(route)||storyRoutes.includes(route)||playerRoutes.includes(route)||collectionRoutes.includes(route)||eventRoutes.includes(route)||visualRoutes.includes(route)?'CONTENTS':fanclubDetailRoutes.includes(route)?'FANCLUB':route==='archive.html'?'ARCHIVE':route.startsWith('member-')?'MEMBERS':active[route]} /><DetailNavigation route={route}/><div id="night-main-content" tabIndex={-1}>{route==='index.html'?<HomePage />:route==='store.html'?<StorePage/>:route==='archive.html'?<SearchArchive />:playerRoutes.includes(route)?<PhotoEpisodePage route={route}/>:collectionRoutes.includes(route)?<CollectionPage route={route}/>:<ArchivePage route={route} />}</div><DetailNavigation route={route} bottom/><Footer /></>;}
