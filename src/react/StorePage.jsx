...
export function StorePage({products,dimensions={}}){
  ...
  return <><main>
    ...
    <section className="store-section section-tight"><div className="container"><div className="store-toolbar"><div aria-label="상품 카테고리" className="store-filters" role="group">{filters.map(([key,label])=><button key={key} className={'store-filter'+(key===filter?' is-active':'')} data-filter={key} type="button" disabled={!ready} aria-pressed={key===filter} onClick={()=>setFilter(key)}>{label}</button>)}</div><span className="store-product-count" id="productCount">{list.length} PRODUCTS</span></div><div className="store-grid" id="storeGrid" aria-live="polite">{list.map(item=>{const thumb=thumbPath(item.images[0]);return <article key={item.id} className="store-card"><button type="button" data-product={item.id} aria-label={`${item.name} 상품 상세 보기`} disabled={!ready} onClick={()=>openProduct(item.id)}><span className="store-card-image"><img src={thumb} alt={item.name} loading="lazy" decoding="async" {...imageProps(dimensions,thumb)}/></span><span className="store-card-meta">{item.category.split(' ')[0].toUpperCase()}</span><h2>{item.name}</h2><strong>{money(item.price)}</strong><small>VIEW DETAILS →</small></button></article>;})}</div></div></section>
    <section className="store-notice section-tight"><div className="container"><span>STORE INFORMATION</span><p>NIGHT의 공식 MD와 라이프스타일 컬렉션을 소개합니다. 가격과 장바구니는 상품 열람을 위한 기능이며, 이 페이지에서는 주문 및 결제를 지원하지 않습니다.</p></div></section>
  </main>
  <div ref={modal} className={'store-modal'+(overlay==='product'?' is-open':'')} id="productModal" aria-hidden={overlay!=='product'} role="dialog" aria-modal="true" aria-label="상품 상세 정보"><div className="store-modal-panel"><button className="store-modal-close" type="button" aria-label="상품 상세 닫기" onClick={()=>setOverlay(null)}>×</button><div className="store-detail-gallery"><img id="detailImage" src={overlay==='product'?product?.images[picture]:undefined} alt={product?.name??''} decoding="async" {...imageProps(dimensions,overlay==='product'?product?.images[picture]:undefined)}/><div className="store-detail-thumbs" id="detailThumbs">{product?.images.map((src,index)=>{const thumb=thumbPath(src);return <button key={src} type="button" className={index===picture?'is-active':''} data-image={src} aria-label={`상품 이미지 ${index+1} 보기`} onClick={()=>setPicture(index)}><img src={thumb} alt="" loading="lazy" decoding="async" {...imageProps(dimensions,thumb)}/></button>;})}</div></div><div className="store-detail-copy"><span id="detailCategory">{product?.category.split(' ')[0].toUpperCase()??''}</span><h2 id="detailName">{product?.name??''}</h2><strong id="detailPrice">{product?money(product.price):''}</strong><p id="detailDescription">{product?.desc??''}</p><div className="store-benefit" id="detailBenefit" hidden={!product?.benefit}>{product?.benefit??''}</div><label htmlFor="detailOption">OPTION</label><select id="detailOption" value={option} onChange={event=>setOption(event.target.value)}>{product?.options.map(value=><option key={value}>{value}</option>)}</select><label htmlFor="detailQuantity">QUANTITY</label><div className="store-quantity"><button id="quantityDown" type="button" aria-label="수량 줄이기" onClick={()=>setQuantity(value=>quantityValue(value-1))}>−</button><input id="detailQuantity" type="number" value={quantity} min="1" max="9" onChange={event=>setQuantity(quantityValue(event.target.value))}/><button id="quantityUp" type="button" aria-label="수량 늘리기" onClick={()=>setQuantity(value=>quantityValue(value+1))}>+</button></div><button className="store-add-button" id="addToCart" type="button" onClick={add}>ADD TO CART</button></div></div></div>
  <aside ref={drawer} className={'store-cart'+(overlay==='cart'?' is-open':'')} id="cartDrawer" aria-hidden={overlay!=='cart'} aria-label="장바구니" role="dialog" aria-modal="true"><div className="store-cart-head"><h2>YOUR CART</h2><button id="closeCart" type="button" aria-label="장바구니 닫기" onClick={()=>setOverlay(null)}>×</button></div><div className="store-cart-items" id="cartItems">{cart.length?cart.map((item,index)=>{const thumb=thumbPath(item.image);return <article key={item.id+':'+item.option} className="store-cart-item"><img src={thumb} alt="" loading="lazy" decoding="async" {...imageProps(dimensions,thumb)}/><div><h3>{item.name}</h3><p>{item.option} · {item.quantity}EA</p><strong>{money(item.price*item.quantity)}</strong></div><button type="button" data-remove={index} aria-label={`${item.name} 장바구니에서 삭제`} onClick={()=>commitCart(cart.filter((_,row)=>row!==index))}>×</button></article>;}) :<p className="store-cart-empty">YOUR CART IS EMPTY.</p>}</div><div className="store-cart-foot"><div><span>SUBTOTAL</span><strong id="cartTotal">{money(total)}</strong></div><button id="checkoutButton" type="button" onClick={()=>notify('이 아카이브에서는 주문 및 결제를 지원하지 않습니다.')}>CHECKOUT</button><small>이 아카이브에서는 주문 및 결제를 지원하지 않습니다.</small></div></aside><div className={'store-backdrop'+(overlay?' is-open':'')} id="storeBackdrop" onClick={()=>setOverlay(null)}></div><div className={'store-toast'+(toast?' is-open':'')} id="storeToast" role="status" aria-live="polite">{toast}</div>
  </>;
}
