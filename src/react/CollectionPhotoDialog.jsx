...
export function CollectionPhotoDialog({photo,onClose,variant='collection'}){
  ...
  if(editorial)return <dialog ref={viewer} id="fv-viewer" aria-label="FIVE VOICES 이미지 확대 보기" onCancel={event=>{event.preventDefault();onClose();}} onClose={onClose} onClick={event=>{if(event.target===event.currentTarget)onClose();}}><div><button className="fv-close" type="button" aria-label="닫기" onClick={onClose}>×</button><img className="fv-full" src={photo?.full} alt={photo?.label??''}/><p className="fv-caption">{photo?.label??''}</p></div></dialog>;
