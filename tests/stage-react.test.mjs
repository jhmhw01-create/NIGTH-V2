...
test('eleven performance and broadcast archives remain in unique React routes',()=>{
  assert.equal(stageRoutes.length,11);assert.equal(new Set(reactRoutes).size,reactRoutes.length);assert.ok(stageRoutes.every(route=>reactRoutes.includes(route)));
});
