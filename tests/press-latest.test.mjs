...
test('PRESS archive includes current releases and documentary in newest-first order', async () => {
  ...
  for(const [id,date,album,track] of [
    ['28','2030-04-08','SOMETIME','CINEMATIC'],
    ['27','2029-12-01','MOMENTS OF THE NIGHT','OFFICIAL DOCUMENTARY'],
  ...
  assert.ok(record);
  for(const album of ['SOMETIME','MOMENTS OF THE NIGHT','INFINITY','PARADOX','REST'])assert.ok(record.searchText.includes(album));
});
