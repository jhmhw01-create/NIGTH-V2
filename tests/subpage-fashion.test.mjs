...
test('every secondary route receives the shared Castle editorial foundation',async()=>{
  ...
  assert.equal(reactRoutes.filter(route=>!indexRoutes.has(route)).length,reactRoutes.length-indexRoutes.size);
  assert.match(build,/subpage-fashion\.css/);assert.match(build,/data-night-subpage/);assert.match(css,/body\[data-night-subpage\]/);assert.match(css,/@media\(max-width:540px\)/);assert.ok(!css.includes('url('));
});
