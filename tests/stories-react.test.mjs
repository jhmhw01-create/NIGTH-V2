...
test('five story archives remain in unique React routes',()=>{
  assert.deepEqual(storyRoutes,['behind.html','travel.html','observation-2027.html','night-off-summer.html','documentary-2029.html']);
  assert.equal(new Set(reactRoutes).size,reactRoutes.length);assert.ok(storyRoutes.every(route=>reactRoutes.includes(route)));
});
test('existing story photos keep their full and thumbnail paths and viewer styles',async()=>{
  let count=0;
  for(const route of storyRoutes){
    ...
    if(route!=='documentary-2029.html')assert.equal(converted.filter(node=>['behindLightbox','travelLightbox','archive26Lightbox'].includes(node.props.id)).length,1);
  }
