...
test('34 text entries keep links, categories and existing deep links',async()=>{
...
test('contents page renders every entry and keeps SOMETIME first in the album group',async()=>{
  ...
  assert.equal(placeholders.length,34);
  assert.equal(new Set(placeholders).size,34);
  assert.deepEqual([...placeholders].sort((a,b)=>a-b),Array.from({length:34},(_,index)=>index));
  assert(placeholders.indexOf(31)<placeholders.indexOf(7));
  assert(page.contentHtml.includes('전체 <span>34</span>'));
