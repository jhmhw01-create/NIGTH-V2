...
test('eight album archives join the 22 unique React routes',()=>{
  assert.equal(albumRoutes.length,8);assert.equal(reactRoutes.length,22);assert.equal(new Set(reactRoutes).size,22);
});
