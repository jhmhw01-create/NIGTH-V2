...
test('ten album archives remain in unique React routes',()=>{
  assert.equal(albumRoutes.length,10);assert.equal(new Set(reactRoutes).size,reactRoutes.length);assert.ok(albumRoutes.every(route=>reactRoutes.includes(route)));
});
