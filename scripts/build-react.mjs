...
import {albumRoutes,stageRoutes,fanclubDetailRoutes,reactRoutes} from '../src/react/routes.mjs';
...
export async function buildReactPages() {
  ...
  for(const route of routes.slice(1)) {
    ...
    trees[route]=pageTree(renderCollections(albumRoutes.includes(route)?albumMarkup(page):stageRoutes.includes(route)||fanclubDetailRoutes.includes(route)?archiveMarkup(page):page.contentHtml,collections));
  }
  ...
  console.log('35 React routes pre-rendered; other 17 routes unchanged.');
}
