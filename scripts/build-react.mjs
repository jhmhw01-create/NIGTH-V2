...
import {albumRoutes,stageRoutes,reactRoutes} from '../src/react/routes.mjs';
...
export async function buildReactPages() {
  ...
  for(const route of routes.slice(1)) {
    ...
    trees[route]=pageTree(renderCollections(albumRoutes.includes(route)?albumMarkup(page):stageRoutes.includes(route)?archiveMarkup(page):page.contentHtml,collections));
  }
  ...
  console.log('30 React routes pre-rendered; other 22 routes unchanged.');
}
