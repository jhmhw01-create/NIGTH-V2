...
import {albumRoutes,stageRoutes,fanclubDetailRoutes,storyRoutes,playerRoutes,collectionRoutes,editorialRoutes,eventRoutes,reactRoutes} from '../src/react/routes.mjs';
...
export async function buildReactPages() {
  ...
  for(const route of routes.slice(1)) {
    ...
    trees[route]=pageTree(renderCollections(albumRoutes.includes(route)?albumMarkup(page):stageRoutes.includes(route)||fanclubDetailRoutes.includes(route)||storyRoutes.includes(route)||editorialRoutes.includes(route)||eventRoutes.includes(route)?archiveMarkup(page):page.contentHtml,collections));
  }
  ...
  console.log('47 React routes pre-rendered; other 5 routes unchanged.');
}
