...
await writeFile(join(root,'dist',route),'<!DOCTYPE html>\n<html '+page.htmlAttributes+'><head>'+page.headHtml+'<link rel="stylesheet" href="assets/css/detail-navigation.css"><link rel="stylesheet" href="assets/css/site-stability.css"><link rel="stylesheet" href="assets/css/discovery-guide.css"><link rel="stylesheet" href="assets/css/readability.css">'+homeTheme+fanclubDetailTheme+listenTheme+albumDetailTheme+'</head><body '+page.bodyAttributes+(fanclubDetailRoutes.includes(route)?' data-night-fanclub-detail="true"':'')+(route==='listen.html'?' data-night-listen="true"':'')+(albumRoutes.includes(route)?' data-night-album-detail="true"':'')+' data-night-surface="'+(route==='index.html'?'home':'information')+'">'+body+'</body></html>\n');
  }
